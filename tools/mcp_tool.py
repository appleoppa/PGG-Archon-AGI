    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        try:
            while True:
                ...
                # 830-tool server). ``ping`` is OPTIONAL, so a tool-capable
                # server that does not implement it falls back to ``list_tools``
                # for this connection rather than reconnect-looping.
                if self.session:
                    try:
