...
class EVMCore:
    ...
    def calculate_evm(self) -> float:
        ...
        # 缺陷率（修复：归一化，缺陷数量/12）
        total_defect_rate = sum(self.defects.values()) / 12.0
        
        # EVM计算（修复：比例相乘）
        evm = modern_product * ancient_product * (1.0 - total_defect_rate)
    ...
    def get_status(self) -> Dict:
        """获取EVM状态"""
        defect_rate = sum(self.defects.values()) / 12.0
        return {
...
if __name__ == "__main__":
    ...
    print(f"缺陷率: {status['defect_rate']:.4f}")
    print()
    print("缺陷详情:")
