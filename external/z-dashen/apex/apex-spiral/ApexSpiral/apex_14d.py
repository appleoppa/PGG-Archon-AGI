...
DEFAULT_PARAMS = {
    ...
    "Ω_self": 1.0,      # 自我意识系数
    "Γ_reflect": 0.0,   # 反思增益
    "ζ": 1.0,           # 工具链成功率（软版几何平均，默认1.0=完美）
    "D_time": 1.0,       # 时间衰减因子（无衰减时=1.0）
}
...
PARAM_INFO = {
    ...
    # V10.4新增
    "Ω_self": {"name": "自我意识", "layer": 4, "range": [0.1, 2]},
    "Γ_reflect": {"name": "反思增益", "layer": 4, "range": [0, 3]},
    "ζ": {"name": "工具链", "layer": 3, "range": [0, 1]},
    "D_time": {"name": "时间衰减", "layer": 3, "range": [0, 1]},
}
...
def calculate_ΔG(params: dict) -> float:
    ...
    V10.4修复:
    - 软保护乘积: 0.85×product + 0.15×mean (防单点归零)
    - Ω_self × (1+Γ_reflect) 打通 (自我意识+反思)
    - ζ 工具链 + D_time 时间衰减 新增维度
    """
    # 核心15参数乘积
    raw_product = (
        params["ΔG_base"] *
        params["Θ"] * params["K"] * params["ε"] *
        params["Φ"] * params["Ψ"] * params["Π"] *
        params["PID"] * params["RD"] * params["Kelly"] *
        params["E_xp"] * params["Γ"] * params["M_meta"] *
        params["Λ_ctx"] * params["Ξ"]
    )
    
    # 软保护: 85%乘积 + 15%均值
    values = [
        params["ΔG_base"], params["Θ"], params["K"], params["ε"],
        params["Φ"], params["Ψ"], params["Π"],
        params["PID"], params["RD"], params["Kelly"],
        params["E_xp"], params["Γ"], params["M_meta"],
        params["Λ_ctx"], params["Ξ"]
    ...
    protected = 0.85 * raw_product + 0.15 * mean_val
    
    # V10.4打通: Ω_self × (1+Γ_reflect)
    awareness = params.get("Ω_self", 1.0) * (1 + params.get("Γ_reflect", 0.0))
    
    # 工具链ζ + 时间衰减D_time
    tool_chain = params.get("ζ", 1.0)
    time_decay = params.get("D_time", 1.0)
    
    return protected * awareness * tool_chain * time_decay
