...
/// 公式通解+技能全域掌握公式 V10.4修复版（δ正则化+K_max截断）
/// K_master = K_code × (1 + Σ min(τ/(1-τ+δ), K_max)) × υ_apply
/// 修复: δ=0.01防奇异，K_max=10防τ=0.99时爆炸
pub fn calculate_k_master_safe(params: &MasterParams) -> f64 {
    ...
    let sum_tau_converged: f64 = params
        ...
        .map(|&t| {
            ...
            (safe_t / (1.0 - safe_t + delta)).min(k_max)
        })
        .sum();
...
/// 全场景自主深度修复公式
/// ε_self-repair = 1 + |(G_target - G_actual) / G_actual| × δ_error-locate × ψ_thorough-fix × κ_no-repeat
/// 全场景自主深度修复公式 V10.4修复版
/// ε_self-repair = 1 + max(0, Gt-Ga) / max(Ga, 1e-6) × δ × ψ × κ
/// 修复1: max(Ga, 1e-6) 替代 =0 判断，防INFINITY崩溃
/// 修复2: max(0, Gt-Ga) 替代 .abs()，超额不扣分
pub fn calculate_self_repair(params: &SelfRepairParams) -> f64 {
    let safe_actual = params.g_actual.max(1e-6);
    let over = (params.g_target - safe_actual).max(0.0);
    let relative_error = over / safe_actual;
...
/// 正向循环反馈增益公式 V10.4修复版（tanh饱和函数）
/// Φ_cycle = 1 + α·tanh(β·ηρ)，α=1.0，β=1.0，饱和到[1.0, 2.0]
/// 修复: tanh替代E.powf(min(7.0))，从1096→2.0可控
pub fn calculate_cycle_gain_safe(params: &CycleParams) -> f64 {
    let alpha: f64 = 1.0;
    let beta: f64 = 1.0;
    1.0 + alpha * (beta * params.eta_skill_up * params.rho_result_feedback).tanh()
}
