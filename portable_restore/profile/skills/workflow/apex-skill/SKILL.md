...
### 4.1 配套 umbrella skill（2026-06-04 落地）

- `pgg-archon-profile-bootstrap` — profile 级 AGENTS.md / MEMORY.md / skills 引导
- `pgg-archon-eval-harness-suite` — redteam / multimodal / benchmark 评测 harness 套件
- `pgg-archon-closed-loop-formula` — 流程闭合总公式（v0.1.0）
- `pgg-archon-evomap-toolchain` — 进化核心驱动（evolver / autoresearch / openhands / superpowers 状态卡）

这些是 L2–L3 composite skill 的释放样例。未来新增 APEX 主公式时，应优先看是否可挂在已有 umbrella 下，而不是新建孤立 skill。
