...
SESSIONS_DIR=""  # Hermes uses session_search tool for session access
MAX_LINES=50  # tail this many lines per session

# Get active sessions — Hermes uses session_search tool
# Legacy: sessions_json=$(openclaw sessions --active "$ACTIVE_MINUTES" --json 2>/dev/null)
