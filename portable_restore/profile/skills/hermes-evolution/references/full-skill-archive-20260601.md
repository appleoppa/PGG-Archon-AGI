...
- `references/evolution-gene-database.md`: 将外部学习/短板修复落地为工作空间进化基因库的 schema、边界和验证清单。**当前 PGG Archon 实际使用的是 flat JSON GeneDB**（`~/.hermes/hermes-agent/workspace/ultimate_evolution_formula/GeneDB/gene_{NNN}.json`，schema `PGGArchonGeneDBEntry/v1`），不是 SQLite。写入规范：必须先计算 SHA256 再填入 `sha256` 字段，禁止用 placeholder 事后替换（会破坏审计链）。
