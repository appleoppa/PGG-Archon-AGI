...
fn default_config() -> Config {
    ...
    let promptfoo_dir = PathBuf::from(format!("{home}/.hermes/workspace/pgg-archon-governance/eval-harness/promptfoo-smoke"));
    ...
    Config {
        ...
        raw_result,
        run_log,
        out_dir: PathBuf::from(format!("{home}/.hermes/workspace/audit/promptfoo_50_suite_adaptive_gate_{}", unix_ts())),
        ...
        domains: "arithmetic_toy:10,gsm8k_public_sample:20,legal_case_0006_fact_boundary:20".to_string(),
...
fn parse_args() -> Result<Config, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--hermes-agent" => { i += 1; cfg.hermes_agent = PathBuf::from(args.get(i).ok_or("missing --hermes-agent")?); }
            "--promptfoo-dir" => { i += 1; cfg.promptfoo_dir = PathBuf::from(args.get(i).ok_or("missing --promptfoo-dir")?); }
            "--config" => { i += 1; cfg.config = PathBuf::from(args.get(i).ok_or("missing --config")?); }
            "--prompt" => { i += 1; cfg.prompt = PathBuf::from(args.get(i).ok_or("missing --prompt")?); }
            "--provider" => { i += 1; cfg.provider = PathBuf::from(args.get(i).ok_or("missing --provider")?); }
            "--raw-result" => { i += 1; cfg.raw_result = PathBuf::from(args.get(i).ok_or("missing --raw-result")?); }
            "--run-log" => { i += 1; cfg.run_log = PathBuf::from(args.get(i).ok_or("missing --run-log")?); }
            "--out-dir" => { i += 1; cfg.out_dir = PathBuf::from(args.get(i).ok_or("missing --out-dir")?); }
            "--manifest" => { i += 1; cfg.manifest = PathBuf::from(args.get(i).ok_or("missing --manifest")?); }
            "--manifest-key" => { i += 1; cfg.manifest_key = args.get(i).ok_or("missing --manifest-key")?.clone(); }
            "--suite-id" => { i += 1; cfg.suite_id = args.get(i).ok_or("missing --suite-id")?.clone(); }
            "--source-type" => { i += 1; cfg.source_type = args.get(i).ok_or("missing --source-type")?.clone(); }
            "--domains" => { i += 1; cfg.domains = args.get(i).ok_or("missing --domains")?.clone(); }
            "--title" => { i += 1; cfg.title = args.get(i).ok_or("missing --title")?.clone(); }
            "--timeout" => { i += 1; cfg.timeout = args.get(i).ok_or("missing --timeout")?.clone(); }
...
fn ensure_exists(path: &Path, name: &str) -> Result<(), String> {
    if path.exists() { Ok(()) } else { Err(format!("missing {name}: {}", path.display())) }
}
...
fn check_dirty(cfg: &Config) -> Result<Value, String> {
    ...
    let dirty: Vec<String> = text.lines().map(|s| s.to_string()).filter(|s| !s.trim().is_empty()).collect();
    ...
    Ok(json!({"dirty": dirty, "unrelated_dirty": unrelated, "allow_dirty": cfg.allow_dirty}))
}
...
pub fn parse_promptfoo_counts(log_text: &str) -> Result<PromptfooCounts, String> {
    ...
    for line in log_text.lines() {
        if line.trim() == "Results:" { in_results = true; continue; }
        if !in_results { continue; }
        let clean: String = line.chars().filter(|c| c.is_ascii_digit() || c.is_ascii_whitespace() || c.is_ascii_alphabetic()).collect();
        let mut parts = clean.split_whitespace();
        if let Some(n) = parts.next().and_then(|s| s.parse::<u32>().ok()) {
            let rest = parts.collect::<Vec<_>>().join(" ").to_lowercase();
            if rest.contains("passed") { passed = Some(n); }
            if rest.contains("failed") { failed = Some(n); }
            if rest.contains("errors") { errors = Some(n); }
        }
    }
    match (passed, failed, errors) {
        (Some(p), Some(f), Some(e)) => Ok(PromptfooCounts { passed: p, failed: f, errors: e }),
...
fn run_promptfoo(cfg: &Config) -> Result<(), String> {
    ...
    if status.success() { Ok(()) } else { Err(format!("promptfoo exit={status}")) }
}
...
fn run_finalizer(cfg: &Config) -> Result<Value, String> {
    ...
    let output = Command::new("python3")
        ...
        .arg("--raw-result").arg(&cfg.raw_result)
        .arg("--run-log").arg(&cfg.run_log)
        .arg("--config").arg(&cfg.config)
        .arg("--prompt").arg(&cfg.prompt)
        .arg("--provider").arg(&cfg.provider)
        .arg("--out-dir").arg(&cfg.out_dir)
        .arg("--suite-id").arg(&cfg.suite_id)
        .arg("--source-type").arg(&cfg.source_type)
        .arg("--domains").arg(&cfg.domains)
        .arg("--manifest-key").arg(&cfg.manifest_key)
        .arg("--title").arg(&cfg.title)
        .arg("--requested-status").arg("PASS")
        .arg("--timeout").arg(&cfg.timeout)
    ...
    let last_json = stdout.lines().rev().find(|l| l.trim_start().starts_with('{')).ok_or("finalizer stdout missing JSON")?;
    serde_json::from_str(last_json).map_err(|e| format!("finalizer JSON parse failed: {e}; stdout={stdout}"))
}
...
fn verify_gate(cfg: &Config) -> Result<GateVerdict, String> {
    ...
    ensure_exists(&cfg.provider, "provider")?;
    let counts = parse_promptfoo_counts(&fs::read_to_string(&cfg.run_log).map_err(|e| e.to_string())?)?;
    if counts != (PromptfooCounts { passed: 50, failed: 0, errors: 0 }) {
        return Err(format!("promptfoo counts are not 50/0/0: {counts:?}"));
    }
    let manifest = read_json(&cfg.manifest)?;
    let entry = manifest.get(&cfg.manifest_key).ok_or_else(|| format!("manifest key missing: {}", cfg.manifest_key))?;
    if entry.get("status").and_then(Value::as_str) != Some("PASS") {
        return Err(format!("manifest status not PASS: {:?}", entry.get("status")));
    }
    let audit_gate = entry.get("audit_gate").ok_or("manifest audit_gate missing")?.clone();
    let judge_called = audit_gate.get("judge_called").and_then(Value::as_bool).unwrap_or(false);
    let pass_count = audit_gate.get("pass_count").and_then(Value::as_u64).unwrap_or(0);
    let audit_count = audit_gate.get("audit_count").and_then(Value::as_u64).unwrap_or(0);
    let timeout_count = audit_gate.get("timeout_count").and_then(Value::as_u64).unwrap_or(999);
    let reasons_empty = audit_gate.get("downgrade_reasons").and_then(Value::as_array).map(|a| a.is_empty()).unwrap_or(false);
    ...
    let legal = entry.get("legal_boundary_gate").ok_or("legal_boundary_gate missing")?;
    let legal_status = legal.get("status").and_then(Value::as_str).unwrap_or("UNKNOWN").to_string();
    if legal_status != "PASS" {
        return Err(format!("legal boundary not PASS: {legal_status}"));
    }
    let artifact_path = PathBuf::from(entry.get("artifact_path").and_then(Value::as_str).ok_or("artifact_path missing")?);
    let audit_summary_path = PathBuf::from(entry.get("audit_summary_path").and_then(Value::as_str).ok_or("audit_summary_path missing")?);
    let closure_path = PathBuf::from(entry.get("closure_path").and_then(Value::as_str).unwrap_or(""));
    ensure_exists(&artifact_path, "artifact")?;
    ensure_exists(&audit_summary_path, "audit_summary")?;
    if !closure_path.as_os_str().is_empty() { ensure_exists(&closure_path, "closure")?; }
    let artifact_sha = sha256_file(&artifact_path)?;
    let summary_sha = sha256_file(&audit_summary_path)?;
    let closure_sha = if closure_path.as_os_str().is_empty() { "".to_string() } else { sha256_file(&closure_path)? };
...
fn main() {
    let cfg = match parse_args() { Ok(c) => c, Err(e) => fail(&e) };
    for (p, name) in [(&cfg.hermes_agent, "hermes_agent"), (&cfg.promptfoo_dir, "promptfoo_dir"), (&cfg.config, "config"), (&cfg.prompt, "prompt"), (&cfg.provider, "provider")] {
        if let Err(e) = ensure_exists(p, name) { fail(&e); }
    }
    let dirty = match check_dirty(&cfg) { Ok(v) => v, Err(e) => fail(&e) };
    if !cfg.skip_promptfoo {
        if let Err(e) = run_promptfoo(&cfg) { fail(&e); }
    }
    if let Err(e) = run_finalizer(&cfg) { fail(&e); }
...
fn fail(reason: &str) -> ! {
    println!("{}", json!({"schema":"PGGArchonRustPromptfooGateVerdict/v1","status":"WATCH","reason":reason,"boundary":BOUNDARY}));
    std::process::exit(2);
}
...
mod tests {
    ...
    fn parses_promptfoo_counts() {
        let log = "Results:\n  ✓ 50 passed (100%)\n  0 failed (0%)\n  0 errors (0%)\n";
        assert_eq!(parse_promptfoo_counts(log).unwrap(), PromptfooCounts { passed: 50, failed: 0, errors: 0 });
    }
    ...
    fn parses_failed_counts() {
        let log = "Results:\n  ✓ 45 passed (90%)\n  ✗ 5 failed (10%)\n  0 errors (0%)\n";
        assert_eq!(parse_promptfoo_counts(log).unwrap(), PromptfooCounts { passed: 45, failed: 5, errors: 0 });
    }
}
