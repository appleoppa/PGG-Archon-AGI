...
case "$(uname -s)" in
  ...
  Linux) LIB_EXT="so" ;;
  *) echo "Unsupported OS for PyO3 install: $(uname -s)" >&2; exit 1 ;;
esac
...
for crate in "${CRATES[@]}"; do
  ...
  src="$BASE/$crate/target/release/lib${crate}.${LIB_EXT}"
