...
Hermes Desktop 默认启动并管理自己的本地后端，也可以通过 **Settings → Gateway → Remote gateway** 连接到另一台机器上运行的 Dashboard（例如 VM、家庭服务器或 Tailscale 后面的主机）。远程后端必须是已经启动且可访问的 `hermes dashboard` 进程；Desktop 只会连接它，不会替你在远端启动服务。

Desktop 的“远程后端已就绪”探针只访问公开的 `GET /api/status`。真实聊天连接还需要通过 `/api/ws`（以及 `/api/pty`）WebSocket，并受认证、Host header 与绑定地址检查保护。把 Dashboard 绑定到非回环地址时会启用认证门禁；请根据部署场景选择 Nous OAuth、自托管 OIDC 或用户名/密码提供商。用户名/密码方式只适合可信 LAN/VPN，不应直接暴露到公网。
...
在 Desktop 中填写 `http://<backend-host>:9119` 作为 **Remote URL**，然后按后端公布的认证提供商登录。若 `/api/status` 显示 `auth_required: true` 且 `auth_providers` 包含预期 provider，Desktop 的登录流程才会进入对应认证路径。
...
创建和管理 [profiles](../profiles.md)——每个 profile 都是隔离的 Hermes 实例，拥有自己的配置、技能和会话。

- **Profile 卡片** — 显示模型/提供商、技能数量、gateway 状态、描述，以及 active、default、alias 等徽章
- **Create** — 填写名称，可选从 default 克隆、完整克隆、不带内置技能、描述和模型；专用 Profile Builder 页面（`/profiles/new`）提供完整流程（模型、MCP、技能）
- **Manage skills & tools** — 跳转到限定在该 profile 范围内的 Skills 页面
- **Set as active** — 切换后续 CLI/gateway 默认使用的粘性 profile（等同 `hermes profile use`）
- **Edit model / description / SOUL** — 在该 profile 内联编辑
- **Rename / Delete** — 仅适用于具名 profile

Profiles API 位于 `/api/hermes/profiles`：`GET /api/hermes/profiles` 会列出 profile 名称、active 状态、模型、alias、avatar 元数据和 gateway 状态；`/api/hermes/profiles/{name}` 及相关子路由处理 profile 详情、avatar、export、rename、runtime status，以及 gateway/runtime restart 操作。不要假设所有 Dashboard API family 都接受任意 `profile` 参数；profile 管理应使用已文档化的 Profiles 路由。
...
## 认证（门禁模式） {#authentication-gated-mode}

当 Dashboard 绑定到非回环地址时，会启用认证门禁。若门禁会启用但没有注册任何 `DashboardAuthProvider`，`hermes dashboard` 会拒绝绑定并显示明确错误；不存在“默认拒绝但仍接受所有请求”的回退。

### 默认提供商：Nous Research {#default-provider-nous-research}

内置 `plugins/dashboard_auth/nous` 插件会在配置了 client ID 时注册名为 `nous` 的 `DashboardAuthProvider`。这是远程主机、VPS 或公网可达 Dashboard 的推荐路径。使用 `hermes dashboard register` 或 Nous Portal 注册 Dashboard，随后在 Desktop 或 Dashboard 中通过 **Sign in with Nous Research** 登录。

### 用户名/密码提供商（无 OAuth IDP） {#usernamepassword-provider-no-oauth-idp}

如果不接入 OAuth 身份提供商，内置 `plugins/dashboard_auth/basic` 插件可注册名为 `basic` 的提供商，用用户名和密码完成认证。至少需要设置 `HERMES_DASHBOARD_BASIC_AUTH_USERNAME` 以及 `HERMES_DASHBOARD_BASIC_AUTH_PASSWORD` 或 `HERMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH`。建议同时设置稳定的 `HERMES_DASHBOARD_BASIC_AUTH_SECRET`，避免重启后所有会话失效。该方式适合可信 LAN/VPN，不适合直接暴露到公网。
