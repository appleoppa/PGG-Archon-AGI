...
def _mock_all_pass(monkeypatch):
    ...
    monkeypatch.setattr("agent.pgg_archon_status_surface.build_pgg_archon_apex_agi_absorption_surface", lambda: _passing_report(schema="PGGArchonApexAGIAbsorptionSurface/v1", status="PASS", ready_candidate_count=0, blocking_failures=[]))
...
def test_pgg_archon_status_surface_includes_diagnostic_surface(monkeypatch):
    ...
    # 18 subsystems from existing status_surface inputs plus SecurityPolicy/AgentEngine surfaces
    assert len(diag.get("subsystems", [])) == 18
