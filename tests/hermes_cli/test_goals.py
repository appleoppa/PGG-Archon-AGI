...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, hermes_home):
        ...
        suffix = str(abs(hash(str(hermes_home))))
        parent_sid = f"parent-sid-{suffix}"
        child_sid = f"child-sid-{suffix}"
        save_goal(parent_sid, GoalState(goal="ship the feature"))
        assert migrate_goal_to_session(parent_sid, child_sid, reason="compression") is True
        child = load_goal(child_sid)
        assert child is not None and child.goal == "ship the feature"
        # Parent row archived (cleared) so only the child is active.
        parent = load_goal(parent_sid)
