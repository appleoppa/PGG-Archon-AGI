...
from agent.pgg_archon_agent_engine_surface import build_pgg_archon_agent_engine_surface
...
from agent.pgg_archon_security_policy_surface import build_pgg_archon_security_policy_surface
...
def build_pgg_archon_status_surface(
    *,
    ...
    security_policy_report: Mapping[str, Any] | None = None,
    agent_engine_report: Mapping[str, Any] | None = None,
    ...
    try:
        security_policy = dict(security_policy_report) if security_policy_report is not None else dict(build_pgg_archon_security_policy_surface())
    except Exception as exc:
        security_policy = {"schema": "PGGArchonSecurityPolicySurface/v1", "status": "ERROR", "warnings": [f"security_policy_unavailable:{type(exc).__name__}"], "agi_completion_claim": False}
    security_policy_status = _status(security_policy.get("status"))
    security_policy_schema_ok = security_policy.get("schema") == "PGGArchonSecurityPolicySurface/v1"
    security_policy_warnings = [str(item) for item in _as_sequence(security_policy.get("warnings"))[:8]]
    try:
        agent_engine = dict(agent_engine_report) if agent_engine_report is not None else dict(build_pgg_archon_agent_engine_surface())
    except Exception as exc:
        agent_engine = {"schema": "PGGArchonAgentEngineSurface/v1", "status": "ERROR", "warnings": [f"agent_engine_unavailable:{type(exc).__name__}"], "agi_completion_claim": False}
    agent_engine_status = _status(agent_engine.get("status"))
    agent_engine_schema_ok = agent_engine.get("schema") == "PGGArchonAgentEngineSurface/v1"
    agent_engine_warnings = [str(item) for item in _as_sequence(agent_engine.get("warnings"))[:8]]
    ...
    diagnostic_subsystems = [
        ...
        {"name": "security_policy", "status": _diag_status(security_policy_status), "details": f"warnings={len(security_policy_warnings)} capabilities={security_policy.get('capability_count')}"},
        {"name": "agent_engine", "status": _diag_status(agent_engine_status), "details": f"warnings={len(agent_engine_warnings)} engines={agent_engine.get('engine_count')}"},
    ...
    signals = {
        ...
        "security_policy_surface_ready": _signal(
            security_policy_schema_ok and security_policy_status in {"PASS", "WATCH"},
            security_policy,
            f"security_policy_status={security_policy_status} warnings={len(security_policy_warnings)}",
        ),
        "agent_engine_surface_ready": _signal(
            agent_engine_schema_ok and agent_engine_status in {"PASS", "WATCH"},
            agent_engine,
            f"agent_engine_status={agent_engine_status} warnings={len(agent_engine_warnings)}",
        ),
        ...
        "no_agi_completion_claims": _signal(
            not any(
                ...
                for item in (unlock, graph, eval_report, golden, autonomy, promotion_guard,
                            ...
                            research_extraction, security_policy, agent_engine, era_report, flow_report, switch_report, co_debate, co_gene,
    ...
    if not security_policy_schema_ok:
        small_bottlenecks.append({"code": "Sec/Schema", "source": "pgg_archon_security_policy_surface", "status": security_policy_status, "action": "reject_or_rebuild_malformed_security_policy_report", "risk": "low"})
    elif security_policy_status in {"ERROR", "UNKNOWN"}:
        small_bottlenecks.append({"code": "Sec/Error", "source": "pgg_archon_security_policy_surface", "status": security_policy_status, "warnings": security_policy_warnings, "action": "repair_security_policy_surface_before_policy_gate_use", "risk": "low"})
    elif security_policy_status == "WATCH" and "SecurityContextMissing" not in security_policy_warnings:
        small_bottlenecks.append({"code": "Sec/Watch", "source": "pgg_archon_security_policy_surface", "status": security_policy_status, "warnings": security_policy_warnings, "action": "review_security_policy_context_before_privilege_reasoning", "risk": "low"})
    if not agent_engine_schema_ok:
        small_bottlenecks.append({"code": "Eng/Schema", "source": "pgg_archon_agent_engine_surface", "status": agent_engine_status, "action": "reject_or_rebuild_malformed_agent_engine_report", "risk": "low"})
    elif agent_engine_status in {"ERROR", "UNKNOWN"}:
        small_bottlenecks.append({"code": "Eng/Error", "source": "pgg_archon_agent_engine_surface", "status": agent_engine_status, "warnings": agent_engine_warnings, "action": "repair_agent_engine_surface_before_engine_capability_use", "risk": "low"})
    elif agent_engine_status == "WATCH" and "NoAgentEnginesDeclared" not in agent_engine_warnings:
        small_bottlenecks.append({"code": "Eng/Watch", "source": "pgg_archon_agent_engine_surface", "status": agent_engine_status, "warnings": agent_engine_warnings, "action": "review_agent_engine_declarations_before_routing_claims", "risk": "low"})
    ...
    return {
        ...
        "summary": {
            ...
            "security_policy_schema_ok": security_policy_schema_ok,
            "security_policy_warnings": security_policy_warnings,
            "security_policy_ring": security_policy.get("ring"),
            "security_policy_capability_count": security_policy.get("capability_count"),
            "agent_engine_status": agent_engine_status,
            "agent_engine_schema_ok": agent_engine_schema_ok,
            "agent_engine_warnings": agent_engine_warnings,
            "agent_engine_count": agent_engine.get("engine_count"),
            "era_status": era_status,
            "flow_status": flow_status,
            "switch_status": switch_status,
        ...
        "security_policy_surface": security_policy,
        "agent_engine_surface": agent_engine,
        "small_bottlenecks": small_bottlenecks,
