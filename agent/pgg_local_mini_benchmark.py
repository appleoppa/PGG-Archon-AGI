...
# ---------------------------------------------------------------------------
# Benchmark 基因结构 (用于 benchmark 测试的状态 schema)
# 注意: 这不是标准基因模板定义。标准模板见 pgg_archon_gene_fusion_engine.py::validate_standard_gene()
# ---------------------------------------------------------------------------
...
def validate_benchmark_gene(gene: Any) -> dict[str, Any]:
    """验证单个基因结构是否符合 benchmark schema。
    注意: 这不是标准基因模板门禁。标准模板门禁见 fusion_engine 的 validate_standard_gene()。
