...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if url.endswith("/anthropic"):
        ...
        logger.debug("Auxiliary client: rewrote base URL %s → %s", redact_sensitive_text(url, force=True), redact_sensitive_text(rewritten, force=True))
        return rewritten
    if base_url_host_matches(url, "api.kimi.com") and url.endswith("/coding"):
        ...
        rewritten = url + "/v1"
        logger.debug("Auxiliary client: rewrote Kimi base URL %s → %s", redact_sensitive_text(url, force=True), redact_sensitive_text(rewritten, force=True))
        return rewritten
    return url
