"""Bounded repair planner for RuntimeOS small bottlenecks.
...
def _repair_for_bottleneck(item: Mapping[str, Any], index: int) -> dict[str, Any]:
    ...
    if source == "case_flow_graph_replay":
        ...
        return {
            "repair_id": f"runtime_small_repair_{index}_case_flow_{next_node}",
            "source": source,
    ...
    if source == "eval_regression_harness":
        return {
            "repair_id": f"runtime_small_repair_{index}_eval_p0",
            "source": source,
            "targets": code.split("/") if code else [],
            "priority": "P0",
            "risk": "low",
    ...
    if source == "golden_regression_harness":
        return {
            "repair_id": f"runtime_small_repair_{index}_golden",
            "source": source,
            "targets": code.split("/") if code else [],
            "priority": "P1",
            "risk": "low",
    ...
    return {
        "repair_id": f"runtime_small_repair_{index}_manual_review",
...
def build_runtime_small_repair_plan(
    ...
    *,
    ...
    report = {
        "schema": "ApexRuntimeSmallRepairPlan/v1",
    ...
    if write_report:
        ...
        out = out_dir / f"{int(time.time())}_runtime_small_repair_plan.json"
        out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
        report["report_path"] = str(out)
    return report


__all__ = ["build_runtime_small_repair_plan"]
